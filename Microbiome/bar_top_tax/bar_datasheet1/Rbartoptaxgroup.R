...
args <- commandArgs()
infilename <- "./phlya-tongdata11.csv"
top <- 12

outfilename <- "groupoutput.pdf"
titlename <- ""
...
datadf$taxlevel = unlist(map(str_split(datadf$taxlevel[0:top], ';'), function(x){x[2]}))
...
group <- substring(pltdf$variable, 2, 3)
pltdf$variable <- group

pltdf <- aggregate(pltdf$value, by=list(group, taxlevel=pltdf$taxlevel), FUN=mean)
colnames(pltdf) <- c("variable", "taxlevel", "value")
