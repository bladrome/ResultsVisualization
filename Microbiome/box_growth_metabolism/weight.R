...
phplot = ggplot(phdf, mapping = aes(x = variable, y = value, fill = group)) + 
...
# without error bar
#phplot = ggplot(phdatadf, mapping = aes(x = position, y = ph, fill = group)) + 
  #geom_col(position = "dodge") + 
  #geom_errorbar(aes(ymin = , ymax = ph + se)) +
  #theme_classic() +
  #labs( y = "pH value", x = "")+
  #scale_fill_manual(values = c(red, blue))
#phplot
...
######################################## PHPHPH
phdatadf <- datadf[,c(2,6, 7, 8)]
phdatadf <- melt(phdatadf, id.vars = c("group"))
phdatadf$variable = gsub("lwph", "Rumen", phdatadf$variable)
phdatadf$variable = gsub("mcph", "Cecum", phdatadf$variable)
phdatadf$variable = gsub("jcph", "Colon", phdatadf$variable)

red = "#E46161"
blue = "#3377AE"

coldf = aggregate(phdatadf, by=list(group=phdatadf$group, variable=phdatadf$variable), FUN=mean)
coldf = coldf[,c(1, 2, 5)]
ggplot() + 
  geom_col(data=coldf, aes(x = factor(variable, levels = c("Rumen", "Cecum", "Colon")), 
                           fill=factor(group, levels = c("Grazing", "Drylot")),
                           y = value),position = position_dodge2()) +
  geom_point(data=phdatadf, 
             fill="white",
             shape=21,
             aes(x = factor(variable, levels = c("Rumen", "Cecum", "Colon")), 
                 color=factor(group, levels = c("Grazing", "Drylot")),
                                y = value), position = position_jitterdodge()) +
  scale_fill_manual(values = c(red, blue)) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 35, vjust = 0.9, hjust = 0.99), 
        legend.title = element_blank(),
...
ggsave(filename = "phpointplot.pdf", width = 4, height = 4)
...
metabolizeplot = ggplot(plotdatadf, mapping = aes(x = variable, y = value, fill = condition)) +
  #geom_col(position = "dodge") +
  #geom_boxplot() + 
  geom_violin() +
...
metaplot  <- function(df, varaible){
  
ggplot(metadatadf, aes(x = factor(position, levels = c("Rumen", "Cecum", "Colon")), 
                       y = varaible, fill = condition)) +
  stat_boxplot(geom = "errorbar",width=0.75, size=boxplotsize)+
  geom_boxplot(size = boxplotsize, outlier.shape = NA) +
  geom_point(position = position_jitterdodge(),  shape=1, size = 1) +
  theme_classic() +
  theme(
    axis.text = element_text(colour = "black"), 
 ...
 # ylim(0.58, 0.9) +
  labs(x = "") +
  #geom_point(position = "jitterdodge", shape=21, size = 1,  colour = "black", fill = "white")  +
  scale_fill_manual(values = c(red, blue))  
}


metaA <- metaplot(metadatadf, metadatadf$Acetic.acid) + ylim(0.58, 0.9)
ggsave(filename = "ColonmetabolizesplotA.pdf", 
       width = width, height = height, scale = scale)

metaB <- metaplot(metadatadf, metadatadf$Propionic.acid) + ylim(0.11, 0.35)
ggsave(filename = "ColonmetabolizesplotP.pdf",
       width = width, height = height, scale = scale)

metaP <- metaplot(metadatadf, metadatadf$Butyric.acid) + ylim(0.025, 0.17)
...
(metaA + metaB + metaP + plot_layout(nrow = 1))  + plot_layout(ncol = 1)
